...
#   All of the operations will be defined for two tables.  To "study" K tables,
#   you "study" all K choose 2 pairs. 
...
# flights connects to weather via origin (the location), and year, month, day 
# and hour (the time).
...
# does not have a single unique key
...
flights2 %>% left_join(airports) # no common x & y so need by
flights2 %>% left_join(airports, by = c("dest", "faa"))# join columns need to be in data
...
#   you might get (un)lucky and find a combination that's unique in your 
#   current data but the relationship might not be true in general.
...
#   latex's common for keys not to match because of data entry errors. 
